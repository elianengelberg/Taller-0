...
- `sim_persona_zoom`: el check «lo de los demás» miraba `getByText("La reunión")` (substring: media pantalla lo cumple); ahora exige la frase. Quedó una falla sin explicar en una corrida (la línea de Ana no apareció en el detalle del historial aunque estaba en la base): en investigación con una copia instrumentada.
